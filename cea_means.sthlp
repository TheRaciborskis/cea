...
{viewerjumpto "Syntax" "cea_means##syntax"}{...}
{viewerjumpto "Description" "cea_means##description"}{...}
{viewerjumpto "Options" "cea_means##options"}{...}
{viewerjumpto "Examples" "cea_means##examples"}{...}
{viewerjumpto "Stored results" "cea_means##results"}{...}
{p 1 14 16 0}{...}
{bf:cea} {hline 2} Cost-effectiveness analysis using unadjusted means{p_end}
{...}
...
{phang2}
{cmd:cea}
{it:costvar} {it:effvar} {ifin}{cmd:,} {cmd:{ul:treat}ment(}{it:tvar}{cmd:)} 
[{it:{help cea_means##ceaopts:ceaopts}}]
...
{phang}
{it:costvar} specifies the variable containing subject-level cost. 

{phang}
{it:effvar} specifies the variable containing the subject-level effect or 
outcome of interest.
...
{syntab:SE}
{synopt :{opth vce(vcetype)}}{it:vcetype} may be {opt boot:strap} or {opt delta}{p_end}

{syntab:Reporting}
{synopt :{opt level(#)}}set confidence level; default is {cmd:level(95)}{p_end}
{synopt :{opt diff:erences}}display estimated differences in cost and effect{p_end}
{synopt :{it:{help cea_means##display:display_options}}}control columns and column
...
monetary benefit (NMB) for a treatment compared to an alternative using the 
difference in means of the cost and outcome variables. 
...
{phang}
{cmd:vce(delta)} and {cmd:vce(bootstrap)} specify how the VCE and, 
correspondingly, the standard errors are calculated. 

{phang2}
{cmd:vce(delta)} specifies that the delta method is applied to the formula
for the ICER and the VCE of the underlying mean estimation. The 
VCE uses the analytically derived variance estimator associated with 
the sample mean. {cmd:vce(delta)} is not allowed 
when {it:costvar} and {it:effvar} are specified as separately-modeled outcomes.

{phang2}
{cmd:vce(bootstrap)} is the default. Bootstrap samples are drawn within strata
...
{phang}
{cmd:differences} specifies that the estimated difference in means of 
{it:costvar} and {it:effvar} between the two treatment groups used to compute
the ICER or NMB also be reported. {cmd:differences} is only available on 
replay with {cmd:vce(}{helpb bootstrap}{cmd:)} standard errors.
{* {{cmd:differences} may not be specified with the net benefit regression form of {cmd:cea}.}}
...
first group{p_end}
{phang2}{cmd:. cea cost qaly, treatment(tgroup)}{p_end}

{pstd}Report differences in cost and effect on replay{p_end}
{phang2}{cmd:. quietly cea cost qaly, treatment(tgroup)}{p_end}
{phang2}{cmd:. cea, differences}{p_end}

{pstd}Compute net monetary benefit for a willingness to pay of $50,000{p_end}
{phang2}{cmd:. cea cost qaly, treatment(tgroup) wtp(50000)}{p_end}
